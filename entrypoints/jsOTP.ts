...
const SUPPORTED_ALGS: number = 4 | 2 | 1;

('use strict');

/**
 * Int_64 is a class for 2 32-bit numbers emulating a 64-bit number
 *
 * @private
 * @class
...
function str2binb(
	...
	existingBin: number[] = [0],
	existingBinLen: number = 0
): { value: number[]; binLen: number } {
	let bin: number[] = existingBin;
	let codePnt: number;
	let binArr: number[] = [];
	let byteCnt: number = 0;
	let i: number;
	let j: number;
	let existingByteLen: number = existingBinLen >>> 3;
...
function bytes2binb(
	...
	let bin: number[] = existingBin || [0];
	const existingByteLen: number = existingBinLen >>> 3;

	for (let i = 0; i < str.length; i += 1) {
		const codePnt: number = str.charCodeAt(i);
		const byteOffset: number = i + existingByteLen;
		const intOffset: number = byteOffset >>> 2;
...
function binb2hex(
	binarray: number[],
	formatOpts: { outputUpper: boolean }
): string {
	const hex_tab: string = '0123456789abcdef';
	let str: string = '';
	const length: number = binarray.length * 4;
	let i: number;
	let srcByte: number;
 ...
 *   string form
 */
function binb2b64(binarray: number[], formatOpts: { b64Pad: string }): string {
	let str: string = '',
		length: number = binarray.length * 4,
		i: number,
		j: number,
		triplet: number,
		offset: number,
 ...
 *   form
 */
function binb2bytes(binarray: number[]): string {
	let str: string = '';
	const length: number = binarray.length * 4;
	let i: number;
	let srcByte: number;
 ...
 */
function getOutputOpts(options: { outputUpper?: boolean; b64Pad?: string }): {
	outputUpper: boolean;
	b64Pad: string;
} {
	const retVal: { outputUpper: boolean; b64Pad: string } = {
		outputUpper: false,
		b64Pad: '=',
	};
	const outputOptions: { outputUpper?: boolean; b64Pad?: string } =
		options || {};
 ...
 */
function getStrConverter(format: string, utfType: string): Function {
	let retVal: Function;
 ...
 */
function rotl_32(x: number, n: number): number {
	return (x << n) | (x >>> (32 - n));
}
 ...
 */
function rotr_64(x: Int_64, n: number) {
	var retVal = null,
 ...
 */
function shr_32(x: number, n: number) {
	return x >>> n;
}
 ...
 */
function shr_64(x: Int_64, n: number) {
	var retVal = null;
 ...
 */
function parity_32(x: number, y: number, z: number) {
	return x ^ y ^ z;
}
 ...
 * @return {number} The NIST specified output of the function
 */
function ch_32(x: number, y: number, z: number) {
	return (x & y) ^ (~x & z);
}
 ...
 */
function ch_64(x: Int_64, y: Int_64, z: Int_64) {
	return new Int_64(
 ...
 * @return {number} The NIST specified output of the function
 */
function maj_32(x: number, y: number, z: number) {
	return (x & y) ^ (x & z) ^ (y & z);
}
 ...
 * @return {Int_64} The NIST specified output of the function
 */
function maj_64(x: Int_64, y: Int_64, z: Int_64) {
	return new Int_64(
 ...
 * @return {number} The NIST specified output of the function
 */
function sigma1_32(x: number): number {
 ...
 * @return {number} The NIST specified output of the function
 */
function gamma0_32(x: number): number {
 ...
 * @return {Int_64} The NIST specified output of the function
 */
function gamma0_64(x: Int_64): Int_64 {
 ...
 * @return {number} The NIST specified output of the function
 */
function gamma1_32(x: number): number {
 ...
 * @return {Int_64} The NIST specified output of the function
 */
function gamma1_64(x: Int_64): Int_64 {
 ...
 */
function safeAdd_32_2(a: number, b: number): number {
 ...
 */
function safeAdd_32_4(a: number, b: number, c: number, d: number): number {
 ...
 */
function safeAdd_64_2(x: Int_64, y: Int_64): Int_64 {
 ...
 */
function safeAdd_64_4(a: Int_64, b: Int_64, c: Int_64, d: Int_64): Int_64 {
	var lsw, msw, lowOrder, highOrder;
 ...
 */
function getH(variant: string): Array<number | Int_64> {
...
function roundSHA1(block: number[], H: number[]): number[] {
	...
	const ch: (x: number, y: number, z: number) => number = ch_32;
	const parity: (x: number, y: number, z: number) => number = parity_32;
	const maj: (x: number, y: number, z: number) => number = maj_32;
	const rotl: (x: number, n: number) => number = rotl_32;
	const safeAdd_2: (x: number, y: number) => number = safeAdd_32_2;
	const safeAdd_5: (
		a: number,
		b: number,
		c: number,
...
var K_sha2: Array<number>, K_sha512: Array<Int_64>;
if (6 & SUPPORTED_ALGS) {
	K_sha2 = [
 ...
 */
type InputFormat = 'HEX' | 'B64' | 'BYTES' | 'TEXT';
...
class JsSHA {
	private processedLen: number = 0;
	private remainder: number[] = [];
	private remainderLen: number = 0;
	private utfType: Encoding;
	private intermediateH: any;
	private converterFunc: any;
	private shaVariant: string;
	private outputBinLen: number;
	private variantBlockSize: number;
	private roundFunc: any;
	private finalizeFunc: any;
	private finalized: boolean = false;
	private hmacKeySet: boolean = false;
	private keyWithIPad: number[] = [];
	private keyWithOPad: number[] = [];
	private numRounds: any;
	private updatedCalled: boolean = false;

	constructor(variant: string, inputFormat: InputFormat, options?: Options) {
		const inputOptions = options || {};
		this.utfType = inputOptions.encoding || 'UTF8';
		this.numRounds = inputOptions.numRounds || 1;

		if (!Number.isInteger(this.numRounds) || this.numRounds < 1) {
		...
		if ('SHA-1' === this.shaVariant && 1 & SUPPORTED_ALGS) {
			...
			if (6 & SUPPORTED_ALGS) {
				this.roundFunc = function (block: any, H: any) {
					return roundSHA2(block, H, this.shaVariant);
				};
				this.finalizeFunc = function (
					remainder: any,
					remainderBinLen: any,
					processedBinLen: any,
					H: any
			...
			if ('SHA-224' === this.shaVariant && 2 & SUPPORTED_ALGS) {
				this.variantBlockSize = 512;
				this.outputBinLen = 224;
			} else if ('SHA-256' === this.shaVariant && 2 & SUPPORTED_ALGS) {
				this.variantBlockSize = 512;
				this.outputBinLen = 256;
			} else if ('SHA-384' === this.shaVariant && 4 & SUPPORTED_ALGS) {
				this.variantBlockSize = 1024;
				this.outputBinLen = 384;
			} else if ('SHA-512' === this.shaVariant && 4 & SUPPORTED_ALGS) {
				this.variantBlockSize = 1024;
		...
		this.intermediateH = getH(this.shaVariant);
	}
	...
	public setHMACKey(
		...
		if (blockByteSize < keyBinLen / 8) {
			keyToUse = this.finalizeFunc(
				keyToUse,
				keyBinLen,
				0,
				getH(this.shaVariant)
			);
	...
	public update(srcString: string): void {
		var convertRet,
			chunkBinLen,
			chunkIntLen,
			chunk,
	...
	public getHash(format: string, options?: any): string {
		var formatFunc, i, outputOptions: any;
		...
		if (false === this.finalized) {
			this.intermediateH = this.finalizeFunc(
				this.remainder,
				this.remainderLen,
				this.processedLen,
				this.intermediateH
	...
	public getHMAC(format: string, options?: any): string {
		var formatFunc, firstHash, outputOptions: any;
...
class Totp {
	...
	getOtp(secret: string, now = new Date().getTime()) {
